...
const API = "https://backend-production-30a4.up.railway.app/api/product/fetchproduct";
 ...
 const navigate = useNavigate();
  ...
  const handleAddToCart = () => {
    if (sessionStorage.getItem("token")) {
      setshowLogin(false);
      fetch("https://backend-production-30a4.up.railway.app/api/cart/addtocart", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "auth-token": sessionStorage.getItem("token"),
    ...
    <div key={alias} className="mt-20 bg-white">
      ...
      <div className="flex lg:flex-row flex-col">
        ...
        <div className="basis-1/3  lg:mr-5 mx-5 ">
          <div className="mt-3">
            <p className="text-base lg:text-xl text-center lg:text-left font-mono font-bold">3M Mobile Wrap</p>
          </div>
          <div className="mt-3">
            <h1 className="text-xl lg:text-2xl text-center lg:text-left font-extrabold">{name}</h1>
          </div>
          <div className="mt-3">
            <p className="lg:text-base text-sm text-center lg:text-left font-normal text-slate-700">
              {description}
            </p>
          </div>
