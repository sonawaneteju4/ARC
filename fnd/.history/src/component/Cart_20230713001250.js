...
const Cart = () => {
      ...
      const fetchCartItems = async () => {
        ...
        try {
          const res = await fetch(
            `${process.env.BASE_API}/api/cart/cartuser`,
            {
              method: "GET",
              headers: {
  ...
  const fetchProductData = useCallback(async () => {
    ...
    try {
      const productDataPromises = cartItems.map((item) =>
        fetch(
          `${process.env.BASE_API}/product/fetchproduct?_id=${item.productId}`
        ).then((res) => res.json())
      );
  ...
  const handleRemoveCart = async (itemId, e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.BASE_API}/cart/removefromcart/${itemId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleQuantityChange = async (itemId, quantity, price) => {
    ...
    try {
      const res = await fetch(
        `${process.env.BASE_API}/cart/updatecart/${itemId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
