...
const Cart = () => {
      ...
      const fetchCartItems = async () => {
        ...
        try {
          const res = await fetch(
            "https://backend-production-30a4.up.railway.app/api/cart/cartuser",
            {
              method: "GET",
              headers: {
  ...
  const fetchProductData = useCallback(async () => {
    ...
    try {
      const productDataPromises = cartItems.map((item) =>
        fetch(
          `https://backend-production-30a4.up.railway.app/api/product/fetchproduct?_id=${item.productId}`
        ).then((res) => res.json())
      );
  ...
  const handleRemoveCart = async (itemId, e) => {
    ...
    try {
      const response = await fetch(
        `https://backend-production-30a4.up.railway.app/api/cart/removefromcart/${itemId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleQuantityChange = async (itemId, quantity, price) => {
    ...
    try {
      const res = await fetch(
        `https://backend-production-30a4.up.railway.app/api/cart/updatecart/${itemId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="mt-20 min-h-screen divide-y-2">
        ...
        <div className="flex lg:flex-row md:flex-col max-sm:flex-col">
          <div className="basis-2/3 p-5 h-[75vh]  lg:scrollbar-thin scrollbar-thumb-sky-600 scrollbar-track-sky-300 overflow-y-scroll dark:scrollbar-thumb-blue-100 dark:scrollbar-track-gray-700 scrollbar-track-rounded-full">
            {fornew ? (
              <div className="flex flex-col items-center justify-center h-96 odd:bg-gray-100 even:bg-gray-200">
                <svg
              ...
              <div>
                {cartItems.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-96 odd:bg-gray-200 even:bg-teal-300p">
                    <svg
                      className="w-24 h-24 mb-4 animate-bounce text-gray-400"
                      fill="none"
                      stroke="currentColor"
          ...
          <div className="basis-1/3 p-5 m-2 ">
            ...
            {/* <div className="mt-2">
              <form className="flex lg:flex-row flex-col lg:justify-between">
                <input
                  className="border rounded-xl text-black px-2 py-1"
