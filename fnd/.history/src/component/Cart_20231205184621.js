...
const Cart = () => {
      ...
      const fetchCartItems = async () => {
        ...
        try {
          const res = await fetch(
            "https://arc-backend-kh2c.onrender.com/api/cart/cartuser",
            {
              method: "GET",
              headers: {
  ...
  const fetchProductData = useCallback(async () => {
    ...
    try {
      const productDataPromises = cartItems.map((item) =>
        fetch(
          `https://arc-backend-kh2c.onrender.com/api/product/fetchproduct?_id=${item.productId}`
        ).then((res) => res.json())
      );
  ...
  const handleRemoveCart = async (itemId, e) => {
    ...
    try {
      const response = await fetch(
        `https://arc-backend-kh2c.onrender.com/api/cart/removefromcart/${itemId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleQuantityChange = async (itemId, quantity, price) => {
    ...
    try {
      const res = await fetch(
        `https://arc-backend-kh2c.onrender.com/api/cart/updatecart/${itemId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
