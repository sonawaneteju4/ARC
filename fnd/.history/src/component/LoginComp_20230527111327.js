...
const LoginComp = ({ link, onClose, onLoginSuccess, setloading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      "https://backend-production-30a4.up.railway.app/api/auth/login",
      {
        method: "POST",
        headers: {
    ...
    if (json.success) {
      ...
      onLoginSuccess();
      setShowModal(true);
      setModalMessage("Welcome to Inote-Secured Note Cloud");
    } else {
      setShowModal(true);
      setModalMessage("Enter Correct Details");
    }
  };
    ...
    <div>
      ...
      {showModal && (
        <AlertModal
          ...
          color={showModal === "success" ? "blue" : "red"}
        >
          <p>{modalMessage}</p>
        </AlertModal>
      )}
