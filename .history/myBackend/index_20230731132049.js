...
app.post('/api/upload', upload.single('uploadimg'), async (req, res) => {
  console.log('Received an image upload request.'); // Add this log statement
  try {
    ...
    const cloudinaryResponse = await axios.post(
      'https://res.cloudinary.com/dcatujmtp/image/upload',
      formData,
      {
    ...
    if (errorOccursDuringImageUpload) {
      return res.status(500).json({ error: 'Failed to upload the image: <specific reason>' });
    }
