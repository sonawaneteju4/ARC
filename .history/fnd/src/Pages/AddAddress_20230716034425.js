...
const AddAddress = ({ cartId, userId,onClose }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BASE_API}/orders/placeorder`, {
        method: "POST",
        headers: {
      ...
      // Add a delay of 1 second before deleting the cart
      await new Promise((resolve) => setTimeout(resolve, 1000));
  
      // Delete the cart
      const deleteCartResponse = await fetch(`${process.env.REACT_APP_BASE_API}/cart/deletecart`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          "auth-token": sessionStorage.getItem("token"),
      ...
      // Handle the delete cart response
      const deleteCartData = await deleteCartResponse.json();
      console.log(deleteCartData); 
  
      onClose();
      // Optionally, you can perform additional actions after placing the order and deleting the cart, such as redirecting the user to the order success page
    } catch (error) {
      // Handle any errors
      console.error(error);
    }
