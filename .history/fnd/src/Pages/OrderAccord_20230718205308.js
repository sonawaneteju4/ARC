...
const OrderAccord = ({ oId, oderDate, gTotal, status, items, address }) => {
  ...
  return (
    <div className="m-2">
      ...
      {isActive && (
        <div className=" bg-white/80 items-center odd:bg-white even:bg-white/50 ">
          <NavLink to={`/order/${oId}`} orderPlace={true}>
            <div className="flex flex-row">
              {items.map((item) => (
                <div className="basis-4/5 rounded-2xl pt-4" key={item.productId}>
                  <ShowImg
              ...
              <div className="basis-1/5 m-3 bg-white/80 rounded-xl">
                <p>Address:</p>
