...
const AddAddress = ({ cartId, userId, onClose }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle the error and show the custom alert here
        displayErrorAlert(response.status);
      }
    } catch (error) {
      console.error(error);
  ...
  const displayErrorAlert = (statusCode) => {
    // You can customize the error message based on the status code or any other error information
    let errorMessage = 'Something went wrong with status code ' + statusCode;
     ...
     <div className="bg-white text-black rounded-xl w-96 py-2">
      {displayErrorAlert && (
        <HandleError
          ...
          onClose={() => setDisplayErrorAlert(false)}
        />
      )}
