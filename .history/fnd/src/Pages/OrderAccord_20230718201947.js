    ...
    <div key={oId} onClick={() => setIsActive(!isActive)}>
      <div className="p-3 flex  bg-teal-900/30 rounded-xl  ">
        <NavLink to={`/order/${oId}`} orderPlace={true}>
          <div className="flex">
            <p className="m" > Order Id: {oId.slice(-10)} </p>
            <p className="m"> Date: {new Date(oderDate).toLocaleString()}</p>
            <p className="m">Total : {gTotal}</p>
            <p className="m">Order Status : {status}</p>
          </div>
        </NavLink>
      </div>
      {isActive && (
