...
const ProductInfo = () => {
  ...
  const { getSingleProduct, isSingleLoading, singleProduct } =
    ...
    const handleBrandChange = (event, value) => {
      if (value !== null) {
        ...
        // Check if the selected device belongs to the new selected brand
        const selectedDeviceExists = filteredDevices.find(
          (device) => device.label === selectedDevice?.label
        );
        if (selectedDeviceExists) {
          setSelectedDevice(selectedDeviceExists);
        } else {
 ...
 const navigate = useNavigate();
  ...
  return (
    <form key={alias} className="mt-16 bg-white">
      ...
      <div className="flex lg:flex-row flex-col">
        ...
        <div className="basis-1/3  lg:mr-5 mx-5 ">
          <div className="mt-3">
            <p style={{ fontFamily: 'var(--default-font)' }} className="text-base underline lg:text-xl text-center lg:text-left font-mono font-bold">3M Mobile Wrap</p>
          </div>
          <div className="mt-3">
            <h1 style={{ fontFamily: 'var(--art-font)' }} className="text-xl lg:text-2xl text-center lg:text-left font-extrabold">{name}</h1>
          </div>
          <div className="mt-3">
            <p style={{ fontFamily: 'var(--content-font)' }} className="lg:text-lg text-sm text-center lg:text-left font-normal text-slate-800">
              {description}
            </p>
          </div>
          ...
          <div className="mt-5 flex my-1 lg:justify-between lg:flex-row flex-col">
            ...
            <div className="lg:mx-2 mx-0 flex justify-center lg:mt-0 mt-2">
              ...
              required
                ...
                sx={{ width: 300 }}
                renderInput={(params) => (
                  <TextField required {...params} label="Device" variant="outlined" />
                )}
