    ...
    <div key={oId} onClick={() => setIsActive(!isActive)}>
      <div className="p-3 flex  bg-teal-900/30 rounded-xl  ">
        <NavLink to={`/order/${oId}`} orderPlace={true}>
          <div className="flex">
            <p className="mx-auto" > Order Id: {oId.slice(-10)} </p>
            <p className="mx-auto"> Date: {new Date(oderDate).toLocaleString()}</p>
            <p className="mx-auto">Total : {gTotal}</p>
            <p className="mx-auto">Order Status : {status}</p>
          </div>
        </NavLink>
      </div>
      {isActive && (
